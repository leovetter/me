...
//************
var _Canvas;
let _frontImageSrc = './src/assets/lyon-fog.jpg';
let _backImageSrc = './src/assets/lyon.jpg';
let _frontImage;
let _backImage;
let _blackMask;
let _mouseX = 0;
...
function addListeners() {
    document.getElementsByTagName('body')[0].addEventListener('mousemove', onCanvasMouseMove);
    document.getElementsByTagName('body')[0].addEventListener('mouseout', onCanvasMouseOut);
}
...
class MaskedImage {
    ...
    draw(x=0,y=0) {
        if(this.hasImg) {
            _Canvas.context.drawImage(this.img,x,y, window.innerWidth, window.innerHeight);
        }
    }
